...
//                      Nov 18, 2013: 1.01 Fix value of TWI_PRESCALE to 12 (instead of 8)
//                                         resulting in a I2C clock frequency of 100KHz (instead of 150KHz)
//                      Jan 16, 2014: 1.02 Fix FINBOARD_CLK_Synth_* functions by
//                                         adding missing selection of I2C mux channel 1
//
//----------------------------------------------------------------
...
uint32_t FINBOARD_I2C_Read_noverbose( uint8_t twiAddr, uint8_t deviceAddr, uint8_t regAddr, uint8_t *pRegData )
{
    ADI_TWI_RESULT result;
    uint32_t bSuccess = 0;
...
void FINBOARD_I2C_Scan(void)
{
    ...
    {
    	regAddr = 0x00;
  		bSuccess = FINBOARD_I2C_Read_noverbose( TWI_ADDRESS, deviceAddr, regAddr, &regData );
    	if ( bSuccess == 1 )
    	{
...
void FINBOARD_LED_Driver_Select0(void) // I2C mux OFF
{
    uint8_t deviceAddr;
    uint8_t regData;
	...
	// Disable all Channels on I2C Multiplexer
	regData = 0;
	FINBOARD_I2C_Mux_Write( TWI_ADDRESS, FINBOARD_I2C_MUX_ADDR, regData );
}

void FINBOARD_LED_Driver_Select1(void) // I2C mux ON (channel 0)
{
    uint8_t deviceAddr;
    uint8_t regData;
...
void FINBOARD_LED_Driver_Select2(void) // I2C mux ON (channel 1)
{
    uint8_t deviceAddr;
    uint8_t regData;
...
void FINBOARD_LED_Driver_Init(void)
{
    uint8_t deviceAddr;
    uint8_t regData;

	// LED Driver Initialization
    printf( "\tLED Driver Initialization\n" );
...
void FINBOARD_LED_Driver_Config( uint8_t intensity )
{
    uint8_t deviceAddr;
    uint8_t regData;
    int i;
...
#if defined(FINBOARD_SHOW_LIGHTING_IN_CONSOLE)
    printf( "\tLED Driver Configuration\n" );
#endif
    deviceAddr=FINBOARD_LED_DRV1_ADDR;
    // Clean any pending fault status by reading STATUS1 register
    FINBOARD_I2C_Read( TWI_ADDRESS, deviceAddr, 0x02, &regData );
...
uint32_t FINBOARD_CLK_Synth_Write( uint8_t regAddr, uint8_t regData )
{
    ...
    // Clock Synthesizer is on channel 1 of I2C mux, need to activate this channel
    FINBOARD_I2C_Mux_Write( TWI_ADDRESS, FINBOARD_I2C_MUX_ADDR, FINBOARD_I2C_MUX_CHANNEL1 );
