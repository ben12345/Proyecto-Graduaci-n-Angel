...
%%La Alianza de Visión Integrada es una coalición de organizaciones desarrolladoras de sistemas embebidos y/o procesamiento de imagenes, fue fundada por Berkeley Design Technology, Inc. (BDTI), una reconocida firma de análisis e ingeniería de visión integrada desde 1992. En los últimos años han estado trabajando para incrustar la tendencia de visión integrada, y con mayor fuerza debido al desarrollo de procesadores potentes de bajo consumo.
...
Para la obtención del gradiente de intensidad se utiliza típicamente el operador de Sobel[], 


%%El Algoritmo fue propuesto por John F. Canny en A computational approach to edge detection. IEEE Transactions
%on Pattern Analysis and Machine Intelligence, 8(6), 1986, pp. 679-698 []. Es visto por muchos autores como la consistente técnica en el área de la localización de bordes[1]. 

%Canny esta definido por varios criterios de funcionamiento óptimo:

%\begin{enumerate}
%\item Detección: Detectar puntos de borde reales se debe maximizar, mientras que la probabilidad de detectar bordes %falsos (ruido) debe reducirse al mínimo. 
%\item Localización: Los bordes localizados deben ser lo más cerca posible de los bordes reales. 
%\item Número de respuestas: un borde real no debe dar lugar a más de un borde detectado.
%\end{enumerate}
