...
int main(int argc, char** argv)
{
    ...
    Mat out;
    vector<KeyPoint> keypoints_training;
    
    FAST(object,keypoints_training,/*threshold*/6, /*nonmaxSupression*/true);
    
    drawKeypoints(object,keypoints_training, out,Scalar::all(-1), DrawMatchesFlags::DEFAULT);
    char escapeKey='k';
    ...
    if(sift){
      void SIFT(object, InputArray mask, vector<KeyPoint>& keypoints, OutputArray descriptors, bool useProvidedKeypoints=false)¶
	...
	if (good_matches.size() >= thresholdGoodMatches)
	{
	    ...
	    //img_matches
	    putText(image, "Object Found", cvPoint(10,50),FONT_HERSHEY_COMPLEX_SMALL, 2, cvScalar(0,0,250), 1, CV_AA);
	    ...
	    rectangle(img_matches, scene_corners[0] + Point2f( object.cols, 0), scene_corners[3] + Point2f( object.cols, 0), Scalar(0, 255, 0), 1, 8, 0);
	}
	else
	{
